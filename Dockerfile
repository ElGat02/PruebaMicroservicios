...
RUN /opt/jboss/wildfly/bin/add-user.sh Haws Haws --silent

# Definir variables de entorno
ENV WILDFLY_USER Haws
ENV WILDFLY_PASS Haws
ENV DS_NAME PostgreSQLDS
ENV DS_USER postgres
ENV DS_PASS postgres
ENV DS_URI jdbc:postgresql://postgres:5432/mascotasdb
...
COPY target/mascotas.war ${JBOSS_HOME}/standalone/deployments/
